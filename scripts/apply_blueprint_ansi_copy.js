// Archivo: scripts/apply_blueprint_ansi_copy.js
// Actualización de Sanitización ANSI y Botón Copiar Log en blueprint-nexus
...
// 1. system_core/dashboard/fleet_ui.js (< 150 líneas)
...
function renderFleetGrid(processes) {
    ...
    grid.innerHTML = processes.map(p => {
        ...
        const cleanPreview = p.lastLog ? stripAnsi(p.lastLog.message) : 'Esperando logs de proceso...';
        ...
        <div class="glass-card p-4 rounded-2xl border \${isOnline ? 'border-emerald-500/40 shadow-[0_0_15px_rgba(16,185,129,0.1)]' : 'border-slate-800'} flex flex-col justify-between space-y-3 font-mono">
            ...
            <div class="flex items-center justify-between gap-1.5 pt-1 border-t border-slate-800/80">
                <div class="flex items-center gap-1">
                    ...
                    <button onclick="restartFleetServer('\${p.id}')" \${isCore ? 'disabled' : ''} class="px-2.5 py-1 bg-amber-500/20 hover:bg-amber-500/30 disabled:opacity-30 disabled:cursor-not-allowed text-amber-300 border border-amber-500/40 rounded-lg text-[10px] font-bold transition-all cursor-pointer">🔄</button>
                </div>
                <div class="flex items-center gap-1">
...
async function restartFleetServer(id) {
    ...
    try {
        const res = await fetch(\`/api/fleet/restart/\${encodeURIComponent(id)}\`, { method: 'POST' });
        const d = await res.json();
...
// 2. Modificar index.html para incorporar el botón [📋 Copiar Log]
        ...
        <div class="bg-slate-900 border border-cyan-500/40 max-w-3xl w-full rounded-2xl p-5 space-y-3 shadow-2xl shadow-cyan-900/20">
            <div class="flex justify-between items-center border-b border-slate-800 pb-2.5">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button onclick="closeFleetLogs()" class="text-slate-400 hover:text-white font-bold text-base">&times;</button>
                </div>
            </div>
            ...
            <div class="flex justify-between items-center pt-2 border-t border-slate-800">
                ...
                <div class="flex items-center gap-2">
                    <button onclick="copyFleetLogs()" class="px-3 py-1 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-xl text-xs font-semibold transition-all">📋 Copiar Todo</button>
                    <button onclick="closeFleetLogs()" class="px-4 py-1 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-xl text-xs font-semibold transition-all">Cerrar Visor</button>
                </div>
            </div>
        </div>
    </div>`;
...
// Escribir fleet_ui.js e index.html
fs.writeFileSync(path.join(BLUEPRINT_DIR, 'system_core/dashboard/fleet_ui.js'), fleetUiCode, 'utf-8');
console.log('✅ fleet_ui.js actualizado con stripAnsi y copyFleetLogs.');

fs.writeFileSync(indexPath, indexContent, 'utf-8');
console.log('✅ index.html actualizado con botón [📋 Copiar Log].');
