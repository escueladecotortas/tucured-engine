...
import { Folder, FileText, Image, Code, File as FileIcon, Download, ExternalLink, ArrowLeft, Upload, Trash2 } from 'lucide-react';
import { useToast } from './Toast';

export default function FileManager({ projectId, rootPath = '' }) {
    const [files, setFiles] = useState([]);
    const [currentPath, setCurrentPath] = useState(''); // Relative path from project root
    ...
    const fetchFiles = (subfolder = '') => {
        setLoading(true);
        const queryParams = new URLSearchParams({ projectId, subfolder: subfolder || undefined });
        ...
        fetch(`/api/nexus/assets/list?${queryParams}`)
            ...
            .then(data => {
                if (Array.isArray(data)) {
                    setFiles(data);
                    // Update current path only on success
                    setCurrentPath(subfolder);
                } else {
                    console.error("Invalid file format", data);
                }
                setLoading(false);
            })
            .catch(err => {
    ...
    const handleFileUpload = async (e) => {
        ...
        formData.append('projectPath', projectId); // Maps to backend project path
        formData.append('subfolder', currentPath || 'assets'); // Default to assets if root

        try {
            const res = await fetch('/api/nexus/upload_asset', {
                method: 'POST',
                body: formData
            });
        ...
        <div className="bg-black/20 border border-white/5 rounded-xl p-6 h-full flex flex-col backdrop-blur-sm relative overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar bg-black/30 rounded-lg border border-white/5">
                {loading ? (
                    ...
                    <div className="p-10 text-center text-gray-600 italic text-xs">Empty Directory</div>
                ) : (
                    <table className="w-full text-left border-collapse">
                        <thead className="sticky top-0 bg-[#0a0a0a] z-10">
                            <tr className="text-[10px] text-gray-500 font-mono uppercase border-b border-white/10">
                                ...
                                <th className="py-2 w-20">Size</th>
                                <th className="py-2 w-16 text-right pr-3">Action</th>
                            </tr>
                        </thead>
                            ...
                            {files.map((file, idx) => (
                                ...
                                >
                                    ...
                                    <td className="py-2 text-right pr-3">
                                        <button
                                            onClick={(e) => { e.stopPropagation(); window.open(`/nexus_archives/${projectId}/${currentPath ? currentPath + '/' : ''}${file.name}`, '_blank'); }}
